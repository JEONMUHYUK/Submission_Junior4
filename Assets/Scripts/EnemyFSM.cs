...
public class EnemyFSM : MonoBehaviour
{
    ...
    [SerializeField]
    private Transform           projectileSpawnPoint;               // 발사체 생성
    [SerializeField]
    private float               attackRange = 5;                    // 공격 범위 ( 이 범위 안에 들어오면 "Attack" 상테로 변경)
    [SerializeField]
    ...
    // 타겟 정보는 적을 생산하는 EnemyMemoryPool에서 파라미터로 넘겨받도록 Awake -> Setup
    // private void Awake()
    ...
    // ChangeState에서 코루틴 실행시 열거형EnemyState에 정의되어있는 상수의 이름으로 실행하기 때문에 상수와 동일한 이름으로 만들어야한다.
    private IEnumerator Idle()
    {
        ...
        while (true)
        {
            ...
            CalculateDistenceToTargetAndSelectState();
            yield return null;
        }
    }
    ...
    private IEnumerator Wander()
    {
        ...
        while (true)
        {
            ...
            // 목표위치에 근접하게 도달하거나 너무 오랜시간 머물러 있으면
            to = new Vector3(navMeshAgent.destination.x, 0, navMeshAgent.destination.z);
            from = new Vector3(transform.position.x, 0, transform.position.z);
            if ( (to - from).sqrMagnitude < 0.01f || currenTime >= maxTime )
            {
                // 상태를 "대기" 로 변경
                ChangeState(EnemyState.Idle);
            }
    ...
    private void CalculateDistenceToTargetAndSelectState()
    {
